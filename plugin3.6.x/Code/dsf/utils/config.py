...
# Make sure we have some sort of keyring
this_keyring = keyring.get_keyring()
if not this_keyring:
...
def store_key(key: SavedKey, value: str):
	...
	if is_running_in_docker():
		...
		try:
			...
			logger.info(f'''Exception set_password {KEYRING_SERVICE_NAME}, {key.value}, {value}''')
			logger.info(f'{e}')
...
def get_key(key: SavedKey):
	...
	if is_running_in_docker():
		...
		response = ''
		try:
			response =  keyring.get_password(KEYRING_SERVICE_NAME, key.value)
			return response
		except Exception as e:
			logger.info(f'''Exception get_password {KEYRING_SERVICE_NAME}, {key.value}''')
			logger.info(f'{e}')
...
def reset_ssl_files():
	...
	for ssl_file in [SSL_CERT_FILE, SSL_CA_FILE]:
		if os.path.exists(ssl_file): #keyring
			os.remove(ssl_file)
